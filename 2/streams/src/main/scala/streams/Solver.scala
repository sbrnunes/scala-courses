...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    val neighbors = neighborsWithHistory _
    val newNeighbors = initial.map(neighbors.tupled).flatMap(newNeighborsOnly(_, explored))
    if (newNeighbors.isEmpty) initial
    else {
      val stream = initial ++ newNeighbors
      from(stream.sortBy(_._2.size), explored ++ newNeighbors.map(tuple => tuple._1).toSet)
    }
  }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    from(neighborsWithHistory(startBlock, List.empty), Set.empty)
  }
