...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def mostRetweeted(tweet1: Tweet, tweet2: Tweet) = if (tweet1.retweets >= tweet2.retweets) tweet1 else tweet2
    mostRetweeted(elem, mostRetweeted(Try(left.mostRetweeted).getOrElse(elem), Try(right.mostRetweeted).getOrElse(elem)))
  }
