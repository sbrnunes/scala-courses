...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy(ch => ch.toLower).map { case (ch, occ) => (ch, occ.length) }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def subsets(occurrences: Occurrences): List[Occurrences] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        val sub = subsets(occurrences.tail)
        (for {
          num <- 1 to occurrences.head._2
          s <- sub
        } yield (occurrences.head._1, num) :: s).toList ++ sub
      }
    }
